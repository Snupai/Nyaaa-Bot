...
{
    internal class PunchCommandHandler : ICommandHandler
    {
        ...
        public PunchCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/sfw/gif/punch";
                ...
                builder.Description = $"**{userName}** punches **{mentionedUser}**";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : PunchCommandHandler", $"Bad request {e.Message}, Command: punch", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandPunch = new SlashCommandBuilder();
            globalCommandPunch.WithName("punch");
            globalCommandPunch.WithDescription("Punch someone.");
            globalCommandPunch.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true);
            return globalCommandPunch.Build();
        }
    }
}
