...
namespace DC_BOT.Commands
{
    internal class nsfwSoloCommandHandler : ICommandHandler
    {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/solo";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwSoloCommandHandler", $"Bad request {e.Message}, Command: i-solo", null)); //WriteLine($"Error: {e.Message}");
        ...
        public SlashCommandProperties Initialize()
        {
            ...
            globalCommandnsfwSolo.WithName("i-solo");
            globalCommandnsfwSolo.WithDescription("solo.");
            globalCommandnsfwSolo.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
