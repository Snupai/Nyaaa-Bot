...
{
    internal class nsfwHandjobCommandHandler : ICommandHandler
    {
        ...
        public nsfwHandjobCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/handjob";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwHandjobCommandHandler", $"Bad request {e.Message}, Command: i-handjob", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwHandjob = new SlashCommandBuilder();
            globalCommandnsfwHandjob.WithName("i-handjob");
            globalCommandnsfwHandjob.WithDescription("handjob.");
            globalCommandnsfwHandjob.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwHandjob.IsNsfw = true;
            return globalCommandnsfwHandjob.Build();
        }
    }
}
