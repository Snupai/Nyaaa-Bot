...
{
    internal class nsfwKuniCommandHandler : ICommandHandler
    {
        ...
        public nsfwKuniCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/kuni";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwKuniCommandHandler", $"Bad request {e.Message}, Command: i-kuni", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwKuni = new SlashCommandBuilder();
            globalCommandnsfwKuni.WithName("i-kuni");
            globalCommandnsfwKuni.WithDescription("kuni.");
            globalCommandnsfwKuni.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwKuni.IsNsfw = true;
            return globalCommandnsfwKuni.Build();
        }
    }
}
