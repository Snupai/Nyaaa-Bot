...
{
    internal class nsfwNekoCommandHandler : ICommandHandler
    {
        ...
        public nsfwNekoCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/neko";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwNekoCommandHandler", $"Bad request {e.Message}, Command: i-neko", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwNeko = new SlashCommandBuilder();
            globalCommandnsfwNeko.WithName("i-neko");
            globalCommandnsfwNeko.WithDescription("neko.");
            globalCommandnsfwNeko.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwNeko.IsNsfw = true;
            return globalCommandnsfwNeko.Build();
        }
    }
}
