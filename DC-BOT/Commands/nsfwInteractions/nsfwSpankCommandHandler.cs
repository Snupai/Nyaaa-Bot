...
{
    internal class nsfwSpankCommandHandler : ICommandHandler
    {
        ...
        public nsfwSpankCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/spank";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwSpankCommandHandler", $"Bad request {e.Message}, Command: i-spank", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwSpank = new SlashCommandBuilder();
            globalCommandnsfwSpank.WithName("i-spank");
            globalCommandnsfwSpank.WithDescription("spank.");
            globalCommandnsfwSpank.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwSpank.IsNsfw = true;
            return globalCommandnsfwSpank.Build();
        }
    }
}
