...
{
    internal class nsfwToysCommandHandler : ICommandHandler
    {
        ...
        public nsfwToysCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/toys";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwToysCommandHandler", $"Bad request {e.Message}, Command: i-toys", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwToys = new SlashCommandBuilder();
            globalCommandnsfwToys.WithName("i-toys");
            globalCommandnsfwToys.WithDescription("toys.");
            globalCommandnsfwToys.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwToys.IsNsfw = true;
            return globalCommandnsfwToys.Build();
        }
    }
}
