...
{
    internal class nsfwWankCommandHandler : ICommandHandler
    {
        ...
        public nsfwWankCommandHandler(ILogger logger)
        {
        ...
        public async Task HandleAsync(SocketSlashCommand command)
        {
            ...
            {
                string result;
                var url = "https://gallery.fluxpoint.dev/api/nsfw/gif/wank";
            ...
            {
                await this._logger.Log(new LogMessage(LogSeverity.Info, "CommandHandler : nsfwWankCommandHandler", $"Bad request {e.Message}, Command: i-wank", null)); //WriteLine($"Error: {e.Message}");
        ...
        {
            SlashCommandBuilder globalCommandnsfwWank = new SlashCommandBuilder();
            globalCommandnsfwWank.WithName("i-wank");
            globalCommandnsfwWank.WithDescription("wank.");
            globalCommandnsfwWank.AddOption("user", ApplicationCommandOptionType.User, "Choose a user.", isRequired: true); //idk
            globalCommandnsfwWank.IsNsfw = true;
            return globalCommandnsfwWank.Build();
        }
    }
}
